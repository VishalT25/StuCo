...
struct NotificationsView: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 0) {
                ...
                if isRefreshing && recentNotifications.isEmpty && upcomingReminders.isEmpty {
                    Spacer()
                    ProgressView()
                    Spacer()
                } else if upcomingReminders.isEmpty && recentNotifications.isEmpty {
                    Spacer()
                    emptyStateView
                    Spacer()
                    Spacer()
    ...
    private func loadInitialNotifications() async {
        if upcomingReminders.isEmpty && recentNotifications.isEmpty {
    ...
    private func loadNotifications(isRefresh: Bool) async {
        ...
        let (loadedUpcoming, loadedRecent) = await (upcomingRemindersTask, recentNotificationsTask)
