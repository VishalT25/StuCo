...
class EventViewModel: ObservableObject {
    ...
    func toggleEventCompleted(_ event: Event) {
        print("🔄 DEBUG toggleEventCompleted: ========================================")
        print("🔄 DEBUG toggleEventCompleted: Called for '\(event.title)'")
        print("🔄 DEBUG toggleEventCompleted: Event ID: \(event.id.uuidString.prefix(8))")
        print("🔄 DEBUG toggleEventCompleted: ExternalId: \(event.externalIdentifier ?? "none")")
        print("🔄 DEBUG toggleEventCompleted: Current isCompleted: \(event.isCompleted)")

        // Check for duplicates BEFORE toggling
        if let externalId = event.externalIdentifier {
            let duplicates = events.filter { $0.externalIdentifier == externalId }
            print("🔄 DEBUG toggleEventCompleted: Found \(duplicates.count) events with externalId '\(externalId)':")
            for (index, dup) in duplicates.enumerated() {
                print("   [\(index + 1)] ID: \(dup.id.uuidString.prefix(8)), isCompleted: \(dup.isCompleted), title: '\(dup.title)'")
            }
        }

        var updated = event
        updated.isCompleted.toggle()
        print("🔄 DEBUG toggleEventCompleted: Toggled to: \(updated.isCompleted)")
        ...
        print("🔄 DEBUG toggleEventCompleted: Update complete")
    }
    ...
    func addCategory(_ category: Category) {
        // Use async version to ensure database persistence
        Task {
            do {
                try await eventManager.addCategoryAsync(category)
                // IMMEDIATE SYNC: Update local arrays right away for instant UI update
                await MainActor.run {
                    self.syncDataFromOperationsManager()
                }
            } catch {
                print("EventViewModel: Failed to add category: \(error)")
            }
        }
    }

    func updateCategory(_ category: Category) {
        // Use async version to ensure database persistence and proper sync
        Task {
            do {
                try await eventManager.updateCategoryAsync(category)
                // IMMEDIATE SYNC: Update local arrays right away for instant UI update
                await MainActor.run {
                    self.syncDataFromOperationsManager()
                }
            } catch {
                print("EventViewModel: Failed to update category: \(error)")
            }
        }
    }
